<div class="border-b-2 border-white group">
    <div class="sticky top-0 z-20 flex flex-row items-center justify-between p-6 bg-white group-hover:bg-neutral-100">
        <label class="font-bold text-black/50">โปรไฟล์ผู้ใช้</label>
        <button id="user-profile-update" class="px-6 py-2 font-bold text-white bg-green-500 rounded-lg hover:bg-green-600">
            บันทึก
        </button>
    </div>
    <table class="w-full text-sm table-auto group-hover:bg-neutral-100">
        <thead class="border-white border-y-2 bg-black/5">
            <tr class="font-medium text-left text-black/50">
                <th class="p-6 h-[90px] w-full">โปรไฟล์ผู้ใช้ <span class="text-xs font-normal text-red-500">(ขนาดขั้นต่ำ 35x35 px หรือมากกว่า)</span></th>
            </tr>
        </thead>
        <tbody id="t-user-profile" class="divide-y-2 divide-white bg-black/5">
            <tr class="text-sm font-medium text-black/75">
                <td class="w-full p-6">
                    <div class="relative overflow-hidden rounded-lg cursor-pointer image-button group w-14 sm:w-28 aspect-square">
                        ...
                        <img src="<?= htmlspecialchars($user_profile['setting_value']) ?>" class="absolute top-0 left-0 z-0 object-cover w-full h-full pointer-events-none image-preview">
                    </div>

                    <input type="file" accept="image/*" class="hidden image-input">
                    <input type="hidden" name="image_id[]" value="<?= $user_profile['id'] ?>">
                    <input type="hidden" name="image_url[]" value="<?= htmlspecialchars($user_profile['setting_value']) ?>">
                </td>
            </tr>
        </tbody>
    </table>

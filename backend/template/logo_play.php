<div class="border-b-2 border-white group">
    <div class="sticky top-0 z-20 flex flex-row items-center justify-between p-6 bg-white group-hover:bg-neutral-100">
        <label class="font-bold text-black/50">โลโก้ เข้าสู่เกม</label>
        <button id="logo-play-update" class="px-6 py-2 font-bold text-white bg-green-500 rounded-lg hover:bg-green-600">
            บันทึก
        </button>
    </div>
    <table class="w-full text-sm table-auto group-hover:bg-neutral-100">
        <thead class="border-white border-y-2 bg-black/5">
            <tr class="font-medium text-left text-black/50">
                <th class="p-6 h-[90px] w-full">โลโก้ เข้าสู่เกม</th>
            </tr>
        </thead>
        <tbody id="t-logo-play" class="divide-y-2 divide-white bg-black/5">
            <tr class="text-sm font-medium text-black/75">
                <td class="w-full p-6">
                    <div class="relative overflow-hidden rounded-lg cursor-pointer image-button group w-14 sm:w-28 aspect-square">
                        ...
                        <img src="<?= htmlspecialchars($logo_play['setting_value']) ?>" class="absolute top-0 left-0 z-0 object-cover w-full h-full pointer-events-none image-preview">
                    </div>

                    <input type="file" accept="image/*" class="hidden image-input">
                    <input type="hidden" name="image_id[]" value="<?= $logo_play['id'] ?>">
                    <input type="hidden" name="image_url[]" value="<?= htmlspecialchars($logo_play['setting_value']) ?>">
                </td>
            </tr>
        </tbody>
    </table>

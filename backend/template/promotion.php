<div class="border-b-2 border-white group">
    <div class="sticky top-0 z-20 flex flex-row items-center justify-between p-6 bg-white group-hover:bg-neutral-100">
        <label class="font-bold text-black/50">โปรโมชั่น</label>
        <div class="flex flex-row items-center justify-center gap-4">
            <button id="toggle-reorder-promotion" class="px-4 py-2 font-bold rounded-lg bg-black/5 text-black/50 hover:bg-black/10">
                จัดอันดับ
            </button>

            <button id="promotion-update" class="px-6 py-2 font-bold text-white bg-green-500 rounded-lg hover:bg-green-600">
                บันทึก
            </button>
        </div>
    </div>
    ...
    <table class="w-full text-sm table-auto group-hover:bg-neutral-100">
        <thead class="border-white border-y-2 bg-black/5">
            <tr class="font-medium text-left text-black/50">
                ...
                <th class="w-full p-6 border-r-2 border-white">โปรโมชั่น <span class="text-xs font-normal text-red-500">(ขนาด 1040x1040 px)</span></th>
                <th class="drag-handle-promotion p-6 w-12 h-[90px] border-r-2 text-center border-white">แสดง</th>
                <th class="drag-handle-promotion p-6 w-12 h-[90px] text-center">
                    <button id="add-promotion" type="button" class="flex items-center justify-center w-10 h-10 text-white bg-green-500 rounded-full hover:bg-green-600">
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" class="fill-white">
                            <path d="M440-440H200v-80h240v-240h80v240h240v80H520v240h-80v-240Z" />
                        </svg>
                    </button>
                </th>
                <th class="drag-handle-promotion w-12 h-[90px] hidden p-6 border-r-2 text-center border-white">
        ...
        <tbody id="tbody-promotion" class="divide-y-2 divide-white bg-black/5">
            <?php
            $promotions = getPromotions();

            foreach ($promotions as $index => $promo):
                $isChecked = $promo['status'] === 1 ? 'checked' : '';
            ?>
                <tr class="text-sm font-medium text-black/75">
                    ...
                    <td class="w-full p-6 border-r-2 border-white">
                        <div class="flex flex-col gap-4">
                            <div class="relative overflow-hidden rounded-lg cursor-pointer image-button group w-14 sm:w-28 aspect-square">
                                ...
                                <img src="<?= htmlspecialchars($promo['setting_value']) ?>" class="absolute top-0 left-0 z-0 object-cover w-full h-full pointer-events-none image-preview">
                            </div>

                            <input type="text" name="promotion_link[]" placeholder="Link" value="<?= $promo['setting_link'] ?>"
                            ...
                            <input type="hidden" name="image_url[]" value="<?= htmlspecialchars($promo['setting_value']) ?>">
                        </div>
                    </td>
                    ...
                    <td class="hidden w-12 p-6 text-center cursor-move drag-handle-promotion">
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" class="fill-black/50">
